...
public class purchase extends JFrame implements ActionListener
{
	...
	purchase(String loginid, String Client)
	{
		session = loginid;
		//basic functionalities
		setSize(1800,1040);
		setLayout(null);
		setTitle("Purchase - Aggarwal & Aggarwal's");
			...
			if(optionPaneResult==JOptionPane.NO_OPTION ){
				...
				for(int m=0;m<loop;m++)
				{
					...
					try{
						...
						Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ag","root","Secureit4321@");
						Statement stmt = conn.createStatement();
						stmt.executeUpdate("Insert into purchase(invoiceNumber,invoiceDate,firmId,productId,productDesc,productQty,productPrice,loginUser,totalValue) values('"+b1+"','"+b2+"','"+b3+"','"+proId[0]+"','"+b4+"','"+b5+"','"+b6+"','"+session+"','"+b7+"')");			
					}
					catch(Exception e){}
					}
	...
	public static void main(String[] args) {
		new purchase("himanshu4319","Nittin Kumar & co.");
	}
}