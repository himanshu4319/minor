...
class edit_user extends JDialog implements FocusListener,ActionListener
{
	...
	edit_user( String adminUser, String firm, String user)
	{

		//database fetching records 
		System.out.println(""+adminUser+" "+firm+" "+user);
		
		try{
			Class.forName("com.mysql.cj.jdbc.Driver");
			Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ag","root","Secureit4321@");
			Statement stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery("Select * from admin_user where owner_name = '"+user+"' and login_user = '"+adminUser+"'");
			while(rs.next())
			{
				id = rs.getInt(1);
				s1 = rs.getString(3);
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		{
			String s11,s22,s33;
			s11=user_S_Name_Input.getText();
			s33=passWordInput.getText();
			try{
				Class.forName("com.mysql.cj.jdbc.Driver");
				Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ag","root","Secureit4321@");
				Statement ins = conn.createStatement();
				ins.executeUpdate("update `ag`.`admin_user` SET `owner_name` = '"+s11+"', `pass` = '"+s33+"' where(`id` = '"+id+"')");
				conn.close();
				j1.showMessageDialog(null,"User updated Successfully","Congratulations",JOptionPane.INFORMATION_MESSAGE);
			}
			catch(Exception e)
			{}
		}	
	...
	/*public static void main(String[] args) {
		new edit_user("himanshu","Aggarwal");
	}*/
}
